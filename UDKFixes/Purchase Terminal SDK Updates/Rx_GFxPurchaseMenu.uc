...
function SelectClassPurchase(GFxClikWidget ButtonGroup)
{
    ...
    {
        LogInternal("Exitting due to button not being selected");
        return;
    }
    ...
    {
        Price = rxPurchaseSystem.GetClassPrice(byte(TeamID), IndexToClass(Data, byte(TeamID)));
    }
    // End:0x3CF
    if(PlayerCredits >= float(Price))
    {
        rxPC.PlaySound(soundcue'RenXPTSoundPurchase');
        rxPC.PurchaseCharacter(TeamID, IndexToClass(Data, byte(TeamID)));
        LogInternal("XXX: " @ string(rxPurchaseSystem.GetFamilyClass(byte(TeamID), Data).default.InvManagerClass));
        LogInternal("XXX2: " @ string(rxPurchaseSystem.GetFamilyClass(byte(TeamID), Data).default.InvManagerClass.default.SidearmWeapons[0]));
        rxPC.CurrentSidearmWeapon = rxPurchaseSystem.GetFamilyClass(byte(TeamID), Data).default.InvManagerClass.default.SidearmWeapons[0];
        rxPC.CurrentExplosiveWeapon = rxPurchaseSystem.GetFamilyClass(byte(TeamID), Data).default.InvManagerClass.default.ExplosiveWeapons[0];
        SetLoadout(true);
        rxPC.SwitchWeapon(0);
        ClosePTMenu(false);
    }
