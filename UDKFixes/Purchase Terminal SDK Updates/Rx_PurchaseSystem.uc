...
var Rx_Building_GDI_VehicleFactory		WeaponsFactory;
var Rx_Building_Nod_VehicleFactory		AirStrip;
var Rx_Building_GDI_InfantryFactory		Barracks;
var Rx_Building_Nod_InfantryFactory		HandOfNod;
...
replication
{
	if( bNetInitial && Role == ROLE_Authority )
		NodInfantryPrices, GDIInfantryPrices, PowerPlants, GDIVehiclePrices,  
			NodVehiclePrices, WeaponsFactory, AirStrip, Barracks, HandOfNod;
}
...
simulated event PostBeginPlay()
{
	...
	ForEach AllActors(class'Rx_Building',building)
	{
		...
		{
			WeaponsFactory = Rx_Building_GDI_VehicleFactory(building);
		}
		else if (Rx_Building_Nod_VehicleFactory(building) != None)
		{
			AirStrip = Rx_Building_Nod_VehicleFactory(building);
		}
		else if (Rx_Building_GDI_InfantryFactory(building) != None)
		{
			Barracks = Rx_Building_GDI_InfantryFactory(building);
		}
		else if (Rx_Building_Nod_InfantryFactory(building) != None)
		{
			HandOfNod = Rx_Building_Nod_InfantryFactory(building);
		}	
...
simulated function int GetClassPrice(byte TeamID, class<Rx_FamilyInfo> InfantryClass)
{
    ...
    return int(float(InfantryClass.default.BasePurchaseCost) * Multiplier);
    //return ReturnValue;    
}
...
function PurchaseCharacter(Rx_Controller Buyer, int TeamID, class<Rx_FamilyInfo> CharacterClass)
{
    ...
    if((IsHighTierClass(CharacterClass)) && AreHighTierPayClassesDisabled(byte(TeamID)))
    {
        return;
    }

    if(FFloor(Rx_PRI(Buyer.PlayerReplicationInfo).GetCredits()) >= (GetClassPrice(byte(TeamID), CharacterClass)))
    {
        Rx_PRI(Buyer.PlayerReplicationInfo).RemoveCredits(float(GetClassPrice(byte(TeamID), CharacterClass)));

        if(IsFree(CharacterClass))
        {
            Rx_PRI(Buyer.PlayerReplicationInfo).SetChar(CharacterClass, Buyer.Pawn, true);
        }
        else
        {
            Rx_PRI(Buyer.PlayerReplicationInfo).SetChar(CharacterClass, Buyer.Pawn, false);
        }
        Rx_Game(WorldInfo.Game).RxLogPub(((((((((("GAME" $ Chr(2)) $ "Purchase;") $ Chr(2)) $ "character") $ Chr(2)) $ string(Rx_PRI(Buyer.PlayerReplicationInfo).CharClassInfo.Name)) $ Chr(2)) $ "by") $ Chr(2)) $ class'Rx_Game'.static.GetPRILogName(Buyer.PlayerReplicationInfo));
        Rx_PRI(Buyer.PlayerReplicationInfo).SetIsSpy(false);
    }
}
...
simulated function bool AreVehiclesDisabled(byte teamID, Controller rxPC)
{
	...
	if (WeaponsFactory == none || AirStrip == none) {
		return true;
	}
