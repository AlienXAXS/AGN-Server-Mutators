...
reliable server function FindAndRepair()
{
	...
	foreach Rx_Game(`WorldInfoObject.Game).AllActors(class'Rx_Vehicle', thisVehicle)
	{
		...
		{
			SendMessageToAllPlayers ("Finding PRI for player Rx_Pawn:" $ string(Rx_Pawn(thisVehicle.Driver)) $ " Controller: " $ string(Rx_Controller(Rx_Pawn(thisVehicle.Driver).Controller)));
			if ( Rx_Pawn(thisVehicle.Driver) == none || Rx_PRI(Rx_Pawn(thisVehicle.Driver).Controller.PlayerReplicationInfo) == none )
				continue;
			
			PlayerReplicationInfo = Rx_PRI(Rx_Pawn(thisVehicle.Driver).Controller.PlayerReplicationInfo);
			...
			SendMessageToAllPlayers ( "Repair pad found: " $ string(thisRepairPad) );
			
			if ( !IsDistanceLessThan(thisRepairPad.Location, thisVehicle.Location, distance) )
				continue;
					
			SendMessageToAllPlayers ( "Player is close enough, starting repair process" );
			...
			if ( thisVehicle.Health < thisVehicle.HealthMax )
			{
				...
				ForEach Rx_Game(`WorldInfoObject.Game).AllActors(class'Rx_Building',building)  //`
				{
					if ( thisVehicle.Driver.GetTeamNum() == TEAM_GDI )
						...
						if ( Rx_Building_Nod_PowerFactory(building) != none && Rx_Building_Nod_PowerFactory(building).IsDestroyed() )
							otherRepairRate = (repairRate / 2);
				}
				
				SendMessageToAllPlayers ( "Repair rate calculated: " $ otherRepairRate );
				
				if ( PlayerReplicationInfo.GetCredits() > creditCostPerTick )
				{
					SendMessageToAllPlayers ( "You got enough credits, keep going..." );
					PlayerReplicationInfo.RemoveCredits(creditCostPerTick);
